#!/usr/bin/env python3
"""
Clean environment path for the micro-publication experiments.
"""
...
from .config import AnisotropyConfig, ObservationConfig, RewardConfig
from .rewards import combine_reward_components, compute_navigation_reward
...
class MicroPublicationSwimCrawl(swimmer.Swimmer):
    """
    Smaller, publication-oriented mixed-media task.
    """
    ...
    def _set_starting_position(self, physics):
        """
        Set an evaluation-friendly starting position.
        """
        ...
        if getattr(self, "force_land_start", False) and self._current_land_zones:
            ...
            _apply_root_xy(start_x, start_y)
            return

        # During land phases, start some training episodes near the land boundary to
        # encourage entry/exit transitions rather than deep-land parking.
...
class MicroPublicationSwimmerEnv:
    def __init__(self, n_links=6, desired_speed=_SWIM_SPEED, time_limit=3000, observation_config=None, anisotropy_config=None, reward_config=None, land_start_probability=0.35):
...
class TonicMicroPublicationWrapper(gym.Env):
    def __init__(self, n_links=6, time_feature=True, desired_speed=_SWIM_SPEED, observation_config=None, anisotropy_config=None, reward_config=None, land_start_probability=0.35):
        super().__init__()
        ...
        base_obs_dim = (n_links - 1) + (n_links * 3)
        env_features = 5
        goal_features = 7
    ...
    @property
    def name(self):
        return f"micro-publication-swimmer-{self.env.action_spec.shape[0]}links"
